...
export function BudgetCard() {
  ...
  const handleMouseLeave = () => setHoveredIndex(3)
  ...
  return (
    <div className="relative w-full rounded-[40px] bg-gradient-to-b from-muted/50 to-muted/60 p-3.5 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15),0_0_0_1px_rgba(255,255,255,0.4)_inset] dark:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.4),0_0_0_1px_rgba(255,255,255,0.05)_inset]">
      {/* Inner highlight */}
      <div
      ...
      <div className="relative overflow-hidden rounded-[28px] bg-card p-7 pb-5 shadow-[0_2px_8px_rgba(0,0,0,0.08),0_0_0_1px_rgba(0,0,0,0.04)] dark:shadow-[0_2px_8px_rgba(0,0,0,0.2),0_0_0_1px_rgba(255,255,255,0.05)]">
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <p className="text-[15px] font-medium tracking-wide text-muted-foreground">Wochenumsatz</p>
          ...
          >
            <defs>
              <linearGradient id="areaGradient" x1="0" y1="0" x2="0" y2="1">
                ...
                <stop offset="50%" stopColor="#E8521A" stopOpacity="0.15" />
                <stop offset="100%" stopColor="#E8521A" stopOpacity="0.02" />
              </linearGradient>
              <linearGradient id="lineGradient" x1="0" y1="0" x2="1" y2="0">
                <stop offset="0%" stopColor="#E8521A" />
                <stop offset="100%" stopColor="#c4411a" />
              </linearGradient>
            ...
            {hoveredIndex !== null && (
              <g className="transition-all duration-150 ease-out">
                <circle cx={getX(hoveredIndex)} cy={getY(weekData[hoveredIndex].value)} r="12" className="fill-card" opacity="0.5" />
                <circle cx={getX(hoveredIndex)} cy={getY(weekData[hoveredIndex].value)} r="8" className="fill-card" stroke="#E8521A" strokeWidth="3" filter="url(#dotGlow)" />
              </g>
            )}

            {/* Day labels */}
            {weekData.map((d, i) => (
              <text key={i} x={getX(i)} y={chartHeight - 8} textAnchor="middle" className="text-[12px] font-medium fill-muted-foreground">
                {d.day}
              </text>
            ))}
          </svg>
