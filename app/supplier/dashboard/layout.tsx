...
export default function SupplierDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const { data } = await supabase.from("suppliers").select("name").eq("user_id", user.id).single();
      setSupplierName(data?.name ?? user.email ?? "Lieferant");
    });
  }, []);
      ...
      <aside style={{
        ...
        <div style={{ padding: "24px 20px 20px", borderBottom: `1px solid ${BORDER}` }}>
          <a href="/marketplace" style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:34, height:34, background:ORANGE, borderRadius:9, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Syne',sans-serif", fontWeight:900, fontSize:16, color:"#fff", flexShrink:0 }}>K</div>
            <div>
              <p style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:15, color:TEXT, letterSpacing:"-0.3px", lineHeight:1 }}>KioskFlow</p>
